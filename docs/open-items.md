...
## The cookie banner is switching off all measurement - one setting, needs sign-off (2 Sep 2026)

**What is wrong.** The cookie banner blocks the Google tag manager from loading at all, not
just the cookies. A visitor who has not clicked Accept sends nothing: no page view, no chat,
no enquiry, not even the anonymous signal Google uses to estimate the rest. Proven on live on
2 September on a paid landing page, with the consent decision cleared and the banner untouched.

**The cause.** The banner product keeps a block-list of web addresses in its own account, and
that list contains `googletagmanager.com`, `google-analytics.com` and `doubleclick.net`. Our
own pages are correct: the denied-by-default consent settings run first, exactly as intended.

**The fix.** One setting in the banner account: Advanced Settings, Google consent mode, turn ON
`Allow Google tags to fire before consent`. No website file changes. No tag-manager release.
Reversible in seconds. Staging and live share one banner account, so it lands on both together.

**Proven safe, in a browser on live.** With the Google addresses taken out of the block-list,
and the visitor decision still unmade: tag manager loads, Google Ads and Analytics tags load,
all four advertising and analytics signals read denied, and **no cookies are written**. Click
Accept and the signals switch to granted and the three cookies appear, only then.

**The decision needed.** Piers to agree, and the compliance owner to sign off, because the one
thing that genuinely changes is what a declining visitor sends: nothing today, an anonymous
cookieless signal afterwards. That is standard Consent Mode behaviour, but it is their call.

**Why it is urgent.** The paused "Search | Liquidation" campaign gets one learning period and
it will learn from whatever it can measure. Chat enquiries recorded fell from about 20 a month
to about 3.5 a month, which is roughly one chat in five being counted.

**Full write-up, including the check to run afterwards:**
`docs/consent-blocking-the-tag-manager-2026-09-02.md`.

**Also worth getting.** The accept share still cannot be measured from this project - no
banner-account login, and the analytics access here covers businessexpert.co.uk only. The
banner account's own consent report gives it directly and is on the free plan.
