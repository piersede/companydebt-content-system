...
- **Standard deployment (agreed 2026-08-07): WP Engine Custom push, staging → production, with the Gravity Forms tables excluded.** Full procedure and caveats: `docs/wpe-custom-push-deployment.md`. In short: Staging → Actions → Push to → Production → Custom; select database tables; deselect every `*_gf_*` table (the leads live in `gf_entry`, `gf_entry_meta`, `gf_entry_notes`); also protect `wp_options` unless a setting was deliberately changed on staging. This is a different, supported operation from the blanket copy above.
