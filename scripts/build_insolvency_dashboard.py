...
/* :visited explicitly, not just the base rule. Browsers restrict which
   properties an author :visited rule may set (color is one of the few
   allowed) and, without an explicit :visited selector, a link that's
   already in the browser's history can paint in the UA default visited
   colour despite an !important base `color` - and getComputedStyle() is
   defined to always report the unvisited value, so this specific bug is
   invisible to any JS-based check and can only be caught by looking at
   actual rendered pixels on an already-visited link. */
.cd-data-hub .cd-sectorlinks__btn:visited { color: #fff !important; }
