"""Restore the lost pre-pack 301 to the Quick Redirects (qppr) plugin.

On 2026-06-10 the pre-pack cluster was consolidated onto the canonical page
/company-rescue-solutions/pre-packs/ (id 8396, live) and the legacy page
what-is-a-pre-pack-administration (id 66834) was trashed + 301'd to it. The
qppr rule for that 301 has since been lost, so the trashed URL now 404s.

This re-adds the single missing rule:

  /what-is-a-pre-pack-administration/ -> /company-rescue-solutions/pre-packs/

Mechanism (same proven path as scripts/datahub/add_flatten_redirects.py):
...
Usage:
    python scripts/add_prepack_redirect.py
"""
...
PHP_TEMPLATE = """<?php
/** One-shot: append pre-pack 301 to qppr. Trigger ?__TRIGGER__=__TOKEN__. Self-deletes. */
