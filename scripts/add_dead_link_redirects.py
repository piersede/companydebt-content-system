"""Add 301s for two internal URLs that were dead on staging AND production.

A full sweep of all 311 drafts on 2026-08-17 checked every one of the 199
distinct root-relative links against staging. Two were genuinely dead, confirmed
by the real "Page not found - Company Debt Ltd" title in a browser (not a
bot-block false positive). Each appeared 49 times, inside the shared "your
options" block copied across the 25 sector guides:
...
Both targets return 200 on staging and on production.

The drafts were corrected in commit 436f465, but those 25 pages cannot be pushed:
they carry an unpushed sector rewrite that fails the gate. A 301 fixes the dead
end for readers now, and also catches inbound links from outside the site, which
editing our own pages never would.

Mechanism (same proven path as scripts/add_prepack_redirect.py): SFTP a one-shot
mu-plugin that appends to the `quickppr_redirects` + `quickppr_redirects_meta`
...
Appends - never clobbers existing rules. Idempotent. STAGING ONLY.

Live must be done by hand in the Quick Redirects admin form: there are no live
SFTP credentials in .env, and uploading one-shot code to production is exactly
the mu-plugin litter CLAUDE.md warns about.

Usage:
    python scripts/add_dead_link_redirects.py
"""
...
PHP_TEMPLATE = """<?php
/** One-shot: append dead-link 301s to qppr. Trigger ?__TRIGGER__=__TOKEN__. Self-deletes. */
