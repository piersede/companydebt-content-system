"""Page config for: Director Protection Hub

Live at /director-protection-hub/ (WP pages 77396).

Adopted into Bernstein by registration only -- no content was regenerated.
Page class ("director-protection-hub" -> "entity_owner") sits in
scripts/page_runtime_metadata.py SLUG_PAGE_CLASS_OVERRIDES.

verification_date is the date the draft was last changed in git, not a claim
that the page has been fact-checked since. It exists so the runtime router can
infer an honest freshness tier.

Slim config -- only the fields the Bernstein pipeline and runtime-pack router
read. Content is read directly from drafts/77396_director-protection-hub.html.
"""

PAGE_CONFIG = {
    'slug': "director-protection-hub",
    'page_type': "hub",
    'wp_page_id': 77396,
    'title': "Director Protection During Company Insolvency: How to Reduce Personal Risk",
    'verification_date': "6 August 2026",
}
