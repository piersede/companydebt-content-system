"""Page config for: Insolvency Service Explained: Role, Powers & What UK Directors Need to Know

Live at /insolvency/what-is-the-insolvency-service/ (WP pages 47550).

Adopted into Bernstein by registration only -- no content was regenerated.
Page class ("what-is-the-insolvency-service" -> "legal_compliance") sits in
scripts/page_runtime_metadata.py SLUG_PAGE_CLASS_OVERRIDES.

verification_date is the date the draft was last changed in git, not a claim
that the page has been fact-checked since. It exists so the runtime router can
infer an honest freshness tier.

Slim config -- only the fields the Bernstein pipeline and runtime-pack router
read. Content is read directly from drafts/47550_what-is-the-insolvency-service.html.
"""

PAGE_CONFIG = {
    'slug': "what-is-the-insolvency-service",
    'page_type': "definition",
    'wp_page_id': 47550,
    'title': "Insolvency Service Explained: Role, Powers & What UK Directors Need to Know",
    'verification_date': "12 May 2026",
}
