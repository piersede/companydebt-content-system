"""Brand comparison page assembler.

Used for pages like "Compare Barclaycard Business Credit Cards",
"Compare American Express Business Credit Cards", "Capital on Tap vs Amex".
Features cards from one issuer or a head-to-head comparison.
"""

from __future__ import annotations
from cc_builder.gutenberg import PageBuilder
from cc_builder.css import (
    CC_TOC_CSS, CC_CARD_CSS, CC_HERO_CSS, CC_COMPARISON_TABLE_CSS,
    CC_TABBED_CARD_CSS, CC_ALSO_CONSIDER_VERTICAL_CSS, generate_editorial_css,
)
from cc_builder.js import (
    CC_TOC_HTML, CC_TOC_JS, CC_TOC_START, CC_COMPARISON_TABLE_JS,
    CC_TABBED_CARD_JS,
)
...
def assemble(config: dict, cards: list[dict], separate_cards: list[dict] | None = None) -> str:
    """Assemble a brand comparison page.

    Args:
        config: Page configuration with 'sections' list.
        cards: Cards being compared (from same issuer or head-to-head).
        separate_cards: Optional additional cards for context.

    Section types (in addition to roundup types):
        side_by_side   - Renders cards in a comparison layout
        bottom_line    - Quick "bottom line" verdict box
    ...
    for section in config['sections']:
        ...
        if stype == 'css':
            ...
            pb.raw_html(CC_TABBED_CARD_CSS)
            pb.raw_html(CC_ALSO_CONSIDER_VERTICAL_CSS)
            if config.get('hero'):
            ...
            pb.group_start(
                'bottom-line',
                'border-left:4px solid #6b3fa0;background-color:#f9f5fb;'
                'padding:18px 20px;border-radius:0 6px 6px 0'
            )
            pb.para(section['text'])
            pb.group_end()
            ...
            pb.raw_html(build_section_divider(section.get('label', 'Card comparison')))
            ...
            # Render cards sequentially (CSS handles visual comparison)
            for card in cards:
                pb.raw_html(build_card_html(card))

        elif stype == 'tabbed_side_by_side':
            from cc_builder.components import build_tabbed_card_html
            for i, card in enumerate(cards):
                pb.raw_html(build_tabbed_card_html(card, card_index=i))

        elif stype == 'tabbed_card_list_separate':
            if separate_cards:
                from cc_builder.components import build_tabbed_card_html
                offset = len(cards)
                for i, card in enumerate(separate_cards):
                    pb.raw_html(build_tabbed_card_html(card, card_index=offset + i))
